...
public class DragEventListener implements MouseListener, MouseMoveListener {
    ...
    public DragEventListener(Window window) {
        this.window = window;
        this.isDragging = false;
        this.cursorCoordinates = new Pair<>();
        this.window.getWindowForDrow().getChart().getPlotArea().addMouseListener(this);
        this.window.getWindowForDrow().getChart().getPlotArea().addMouseMoveListener(this);
    }
    ...
    public void mouseMove(MouseEvent mouseEvent) {
        if (this.isDragging) {
            ...
            IAxis xAsix = this.window.getWindowForDrow().getChart().getAxisSet().getXAxis(0);
            IAxis yAsix = this.window.getWindowForDrow().getChart().getAxisSet().getYAxis(0);
